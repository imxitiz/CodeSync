  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      socketRef.current.on(
        ACTIONS.JOINED,
        ({ clients, username, socketId, roomcreator, roomCodeId, code }) => {
          setClients(clients);
    ...
    <div className={`mainWrap ${isSidebarVisible ? '' : 'collapsed'}`}>
      <div className={`aside ${isSidebarVisible ? '' : 'hidden'}`}>
            ...
            {clients
              ...
              .map(({ socketId, username }) => (
                <div
                  key={socketId}
                  onDoubleClick={
                    roomCreator === userName ? () => handleUserDoubleClick(username) : undefined
                  }
                >
        ...
        <div className="controlbuttons">
          {(currentEditor === userName || (roomCreator === userName && currentEditor !== '')) && (
            <button
              className={`btn togglebtn copybtn ${currentEditor === userName && 'leavebtn'}`}
      ...
      <div className="editorWrap">
        ...
        <div className="sidebarToggler" onClick={toggleSidebarVisibility}>
          {isSidebarVisible ? <RiSidebarFoldFill size={30} /> : <RiSidebarUnfoldFill size={30} />}
        </div>
        <Editor
          socketRef={socketRef}
          roomId={id}
          ...
          firstTimesentCode={firstTimesentCode}
        />
      </div>
    </div>
  );
